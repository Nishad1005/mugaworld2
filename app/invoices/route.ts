...
import { assertInvoiceIssuer } from './_auth';
import { next10 } from './_counter';
...
export async function POST(req: Request) {
  ...
  const invoice_no = await next10('invoice', invDate);

  // compute totals
  let subtotal = 0, tax_cgst = 0, tax_sgst = 0, tax_igst = 0;

  const computedItems = items.map((l) => {
    const discount = Number(l.line_discount || 0);
    const net = l.unit_price * l.quantity - discount;
    const taxAmt = (l.tax_rate / 100) * net;

    if (l.tax_type === 'cgst_sgst') {
      tax_cgst += taxAmt / 2;
      tax_sgst += taxAmt / 2;
    } else if (l.tax_type === 'igst') {
      tax_igst += taxAmt;
    }
  ...
  const invoicePayload = {
    ...
    place_of_supply: body.place_of_supply,           // REQUIRED by our spec
  ...
  if (computedItems.length > 0) {
    const rows = computedItems.map(ci => ({ ...ci, invoice_id: inv.id }));
    const { error: itemsErr } = await supabase.from('invoice_items').insert(rows);
    if (itemsErr) return NextResponse.json({ error: itemsErr.message }, { status: 400 });
  }

  return NextResponse.json({ id: inv.id, invoice_no });
}
